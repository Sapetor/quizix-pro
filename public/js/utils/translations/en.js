export default {
    app_title: 'Quizix Pro',

    // User accounts (optional) — auth modal, user chip, visibility
    auth_signin_short: 'Sign in',
    auth_signin_tooltip: 'Sign in to your account',
    auth_title: 'Sign in',
    auth_hint: 'Accounts are optional. Sign in to keep your quizzes private.',
    auth_tab_login: 'Log in',
    auth_tab_signup: 'Sign up',
    auth_username: 'Username',
    auth_password: 'Password',
    auth_submit_login: 'Log in',
    auth_submit_signup: 'Create account',
    auth_signed_in_as: 'Signed in as',
    auth_logout_tooltip: 'Click to log out',
    auth_logout_confirm: 'Log out?',
    auth_error_generic: 'Something went wrong. Please try again.',
    auth_error_required: 'Username and password are required.',
    error_username_required: 'Username is required.',
    error_username_invalid: 'Username must be 3–32 characters: letters, digits, underscore.',
    error_username_taken: 'That username is already taken.',
    error_password_too_short: 'Password must be at least 8 characters.',
    error_password_too_long: 'Password is too long.',
    error_invalid_credentials: 'Invalid username or password.',
    error_rate_limited: 'Too many attempts. Please try again in a minute.',
    error_signup_failed: 'Sign up failed.',
    error_login_failed: 'Login failed.',
    error_auth_required: 'You must be signed in to do that.',
    error_quiz_forbidden: 'You do not have access to this quiz.',
    error_quiz_not_owner: 'You do not own this quiz.',
    error_quiz_legacy_public: 'This quiz is public and cannot be made private.',
    error_invalid_visibility: 'Invalid visibility value.',
    quiz_visibility_private: 'Private',
    quiz_visibility_public: 'Public',
    quiz_visibility_make_public: 'Make public',
    quiz_visibility_make_private: 'Make private',
    quiz_owned_by_you: 'Your quiz',
    filter_show_only_mine: 'Show only my quizzes',
    filter_show_only_mine_tooltip: 'Only show quizzes you created. Uncheck to also see public quizzes.',
    filter_search_placeholder: 'Search quizzes…',
    filter_search_clear: 'Clear search',

    // Header contextual breadcrumb
    header_editor: 'Editor',
    header_untitled_quiz: 'Untitled quiz',

    welcome_to: 'Welcome to',
    skip_to_content: 'Skip to main content',
    host_game: 'Host a Game',
    quick_start: 'Quick Start',
    quick_start_select: 'Select Quiz to Start',
    quick_start_hint: 'Double-click a quiz to start hosting immediately',
    join_game: 'Join Game',
    start_game: 'Start Game',
    create_lobby: 'Create Lobby',
    header_pause: 'Pause',
    header_pause_disabled: 'Pause — coming soon',
    header_end_game: 'End game',
    click_to_copy: 'Click to copy',
    ready_to_start: 'Ready to Start?',
    game_info: 'Game Info',
    quiz_title: 'Quiz Title',
    add_question: 'Add Question',
    save_quiz: 'Save Quiz',
    import_quiz: 'Import Quiz',
    preview_quiz: 'Preview Quiz',
    more_options: 'More Options',
    no_options: 'No options',
    scroll_to_top: 'To Top',
    to_bottom: 'To Bottom',
    create_quiz: 'Create Your Quiz',
    live_preview: 'Live Preview',
    toggle_live_preview: 'Live Preview',
    close_live_preview: 'Close Live Preview',
    ai_generator: 'AI Generator',
    toolbar: 'Toolbar',
    randomize_questions: 'Randomize question order',
    randomize_answers: 'Randomize answer positions',
    use_same_time: 'Use same time for all questions',
    manual_advancement: 'Manual question advancement (host control)',
    manual_advancement_desc: 'Manual advancement is now the default. Uncheck for automatic timer-based advancement',
    browse_games: 'Browse Available Games',
    refresh: 'Refresh',
    back_to_join: 'Back to Join',
    return_to_main: 'Return to Main Menu',
    game_pin: 'Game PIN',
    share_game: 'Share Game',
    players_joined: 'Players Joined',
    students_connected: 'Students connected',
    in_the_room: 'in the room',
    waiting: 'Waiting',
    pin: 'PIN',
    waiting_for_game: 'Waiting for game to start...',
    you_are_in: "You're in!",
    you_are_in_name: "You're in, {name}!",
    enter_game_pin: 'Enter Game PIN',
    your_name: 'Your Name',
    or: 'OR',
    available_games: 'Available Games',
    no_games_found: 'No active games found',
    ask_someone_host: 'Ask someone to host a game or create your own!',
    failed_load_games: 'Failed to load games',
    check_connection: 'Please check your connection and try again.',
    checking_connection: 'Checking...',
    connected: 'Connected',
    offline: 'Offline',
    status: 'Status',
    ping: 'Ping',
    quality: 'Quality',
    players: 'players',
    questions: 'questions',
    questions_total: 'Questions',
    next_question: 'Next Question',
    submit_answer: 'Submit Answer',
    true: 'TRUE',
    false: 'FALSE',
    enter_answer: 'Enter your answer',
    question: 'Question',
    of: 'of',
    welcome: 'Welcome',
    waiting_for_players: 'Waiting for players...',
    in_progress: 'In Progress',
    connected_waiting: 'Connected - Waiting for host',
    game_starting: 'Game is starting!',
    correct_answer_msg: 'Correct!',
    incorrect_answer_msg: 'Incorrect!',
    partially_correct: 'Partially Correct!',
    submitting: 'Submitting...',
    waiting_for_results: 'Waiting for results...',
    points: 'points',
    position: 'Position',
    final_score: 'Final Score',
    leaderboard: 'Leaderboard',
    game_finished: 'Game Finished',
    play_again: 'Play Again',
    new_game: 'New Game',
    exit_to_main: 'Exit to Main Menu',
    stop_quiz: 'Stop Quiz',
    end_round: 'End Round',
    back_to_home: 'Back to Home',
    confirm_stop_quiz: 'Stop the quiz and show results?',
    confirm_back_to_home: 'Leave the game? All players will be disconnected.',
    question_x_of_y: 'Question {0} of {1}',
    option_letter_a: 'A',
    option_letter_b: 'B',
    option_letter_c: 'C',
    option_letter_d: 'D',
    option_letter_e: 'E',
    option_letter_f: 'F',
    time_up_msg: "Time's up!",
    all_players_answered: 'All players answered!',
    waiting_host_advance: 'Waiting for host to advance...',
    share_with_qr_code: 'Share with QR Code',
    scan_to_join: 'Scan to join',
    copy_game_url: 'Copy Game URL',
    generating_questions: 'Generating questions...',
    questions_generated: 'Questions generated successfully',
    successfully_generated_questions: 'Successfully added {0} question(s) to your quiz',
    error_generating: 'Error generating questions',
    select_question_preview: 'Select a question to preview',
    generating_qr: 'Generating QR Code...',
    qr_size: 'QR size',
    qr_size_smaller: 'Make QR code smaller',
    qr_size_larger: 'Make QR code bigger',
    no_questions: 'No questions',
    enter_question_preview: 'Enter your question above to see preview',
    add_question_preview: 'Add a question to see preview',
    please_enter_quiz_title: 'Please enter a quiz title',
    please_add_one_question: 'Please add at least one question',
    question_missing_text: 'Question {0} is missing question text',
    question_needs_two_options: 'Question {0} needs at least 2 valid options',
    please_enter_pin_and_name: 'Please enter both game PIN and your name',
    pin_must_be_six_digits: 'Game PIN must be exactly 6 digits',
    name_max_twenty_chars: 'Name must be 20 characters or less',
    not_connected_refresh: 'Not connected to server. Please refresh the page.',
    please_enter_valid_number: 'Please enter a valid number',
    please_select_at_least_one: 'Please select at least one answer',
    please_select_csv_json: 'Please select a CSV or JSON file',

    // User Manual
    quick_start_guide: 'Quick Start Guide',
    host_steps_title: 'To Host a Quiz:',
    host_step_1: "Click 'Host a Game'",
    host_step_2: 'Create questions or use AI Generator',
    host_step_3: 'Share the Game PIN with players',
    host_step_4: 'Start the game when ready',
    join_steps_title: 'To Join a Quiz:',
    join_step_1: "Click 'Join Game'",
    join_step_2: 'Enter the 6-digit Game PIN or scan QR code',
    join_step_3: 'Enter your name and join game',
    join_step_4: 'Or browse and select from open lobbies',
    features_title: 'Key Features:',
    feature_ai: 'AI question generation',
    feature_realtime: 'Real-time multiplayer',
    feature_math: 'LaTeX math support',
    feature_images: 'Image questions',
    feature_multilang: 'Multi-language support',
    feature_types: 'Multiple question types',
    invalid_file_format: 'Invalid file format. Please check the file structure.',
    error_importing_file: 'Error importing file. Please check the file format.',
    quiz_saved_successfully: 'Quiz saved successfully as {0}',
    failed_save_quiz: 'Failed to save quiz',
    failed_load_quiz_list: 'Failed to load quiz list',
    quiz_loaded_successfully: 'Quiz "{0}" loaded successfully!',
    failed_load_quiz: 'Failed to load quiz',
    failed_upload_image: 'Failed to upload image',

    // Question type dropdown keys (used in editor)
    multiple_choice: 'Multiple Choice',
    true_false: 'True/False',
    multiple_correct: 'Multiple Correct Answers',
    multiple_correct_instruction: 'Select all correct answers - more than one option may be correct!',
    numeric: 'Numeric Answer',
    ordering: 'Ordering',
    ordering_instruction: 'Enter items in the correct order (top to bottom):',
    ordering_player_instruction: 'Use the arrows to arrange the items in the correct order',
    ordering_move_up: 'Move up',
    ordering_move_down: 'Move down',
    ordering_item_1: 'First item',
    ordering_item_2: 'Second item',
    ordering_item_3: 'Third item',
    ordering_item_4: 'Fourth item',
    ordering_needs_two_items: 'Ordering questions need at least 2 items',
    invalid_ordering: 'Invalid ordering configuration',
    please_arrange_items: 'Please arrange the items before submitting',

    // Difficulty dropdown keys (used in editor)
    easy: 'Easy',
    medium: 'Medium',
    hard: 'Hard',

    // Question editor keys
    enter_question: 'Enter your question (supports LaTeX: $x^2$ and code: ```python```)',
    time_seconds: 'Time (s)',
    question_settings: 'Question Settings',
    correct_answer: 'Correct Answer',
    tolerance: 'Tolerance (+/-)',
    explanation_optional: 'Explanation (optional)',
    explanation_placeholder: 'Explain why the correct answer is correct...',
    enter_question_with_latex: 'Enter your question (supports LaTeX and code blocks)',
    option_a: 'Option A',
    option_b: 'Option B',
    option_c: 'Option C',
    option_d: 'Option D',
    add_image: 'Add Image',
    remove_image: 'Remove Image',
    remove: 'Remove',
    enter_numeric_answer: 'Enter numeric answer',

    // Theme and language selector keys
    theme: 'Theme',
    language_en: 'English',
    language_es: 'Spanish',
    language_pl: 'Polish',
    language_fr: 'French',
    language_de: 'German',
    language_it: 'Italian',
    language_pt: 'Portuguese',
    language_ja: 'Japanese',
    language_zh: 'Chinese',

    // Tooltip keys
    start_quiz_tooltip: 'Start the quiz with current questions',
    toggle_fullscreen_tooltip: 'Toggle fullscreen mode',
    toggle_dark_mode_tooltip: 'Toggle dark/light theme',
    adjust_font_size_tooltip: 'Adjust font size',
    manual_advancement_tooltip: 'Manual advancement is now the default. Uncheck for automatic timer-based advancement',
    add_question_tooltip: 'Add a new question',
    remove_question_tooltip: 'Remove question',
    save_quiz_tooltip: 'Save the current quiz',
    load_quiz_tooltip: 'Load a saved quiz',
    ai_generator_tooltip: 'Generate questions with AI',
    toggle_preview_tooltip: 'Toggle live preview',
    import_quiz_tooltip: 'Import quiz from file',
    export_quiz_tooltip: 'Export quiz to file',
    back_to_top_tooltip: 'Scroll to top',
    go_to_bottom_tooltip: 'Scroll to bottom',
    preview_settings_tooltip: 'Preview settings',

    // Additional functionality keys
    loading_games: 'Loading games...',
    loading_quizzes: 'Loading quizzes...',
    no_quizzes_found: 'No quizzes found',
    error_loading_quizzes: 'Error loading quizzes',
    error_saving_quiz: 'Error saving quiz',
    error_loading_quiz: 'Error loading quiz',
    answer_submitted: 'Answer submitted',
    your_answer: 'Your answer',

    // Answer option keys (used in editor)
    a_is_correct: 'A is correct',
    b_is_correct: 'B is correct',
    c_is_correct: 'C is correct',
    d_is_correct: 'D is correct',

    // Additional missing keys
    page_title: 'Quizix Pro - Interactive Quiz Platform',
    question_number: 'Question Number',
    leaderboard_title: 'Leaderboard',
    cancel: 'Cancel',
    save: 'Save',
    edit: 'Edit',
    close: 'Close',

    // Player name change
    name_changed_success: 'Name updated!',
    name_invalid_characters: 'Name contains invalid characters',
    name_is_required: 'Name is required',
    bottom: 'Bottom',
    top: 'Top',
    normal: 'Normal',
    settings: 'Settings',
    navigate: 'Navigate',
    hide: 'Hide',
    reset: 'Reset',
    submit: 'Submit',
    next: 'Next',
    previous: 'Previous',

    // Navigation and accessibility
    go_to_slide_1: 'Go to slide 1',
    go_to_slide_2: 'Go to slide 2',
    go_to_slide_3: 'Go to slide 3',
    adjust_font_size: 'Adjust Font Size',
    spacing: 'Spacing',
    button_size: 'Button Size',
    split_ratio: 'Split Ratio',
    preview_settings_title: 'Preview Settings',
    question_preview: 'Question Preview',
    edit_question: 'Edit Question',
    back_to_top: 'Top',
    quick_actions: 'Quick Actions',
    global_time_limit: 'Global Time Limit',
    mixed: 'Mixed',
    quiz_complete: 'Quiz Complete',
    game_complete: 'Game Complete',
    your_final_rank: 'Your Final Rank',
    top_players: 'Top Players',
    your_concept_mastery: 'Your Concept Mastery',
    live_answer_statistics: 'Live Answer Statistics',
    time_remaining: 'Time remaining',
    player_rankings: 'Player rankings',
    player_answers: 'Player Answers',
    no_answers_yet: 'No answers yet',
    more_answers: 'more answers',
    loading_next_question: 'Loading next question...',
    responses: 'responses',
    of_players_responded: 'of players responded',
    question_types: 'Question Types',
    multiple_choice_type: 'Multiple Choice Type',
    true_false_type: 'True/False Type',
    multiple_correct_type: 'Multiple Correct Type',
    numeric_answer_type: 'Numeric Answer Type',
    ordering_type: 'Ordering',
    difficulty_level: 'Difficulty Level',
    number_of_questions: 'Number of Questions',
    source_material: 'Source Material',
    choose_ai_provider: 'Choose AI Provider',
    ollama_free_local: 'Ollama (Free, Local)',
    openai_paid: 'OpenAI (Paid)',
    claude_paid: 'Claude (Paid)',
    gemini_paid: 'Gemini (Paid)',
    huggingface_free: 'HuggingFace (Free)',
    choose_model: 'Choose Model',
    select_ollama_model: 'Select Ollama Model',
    choose_claude_model: 'Choose Claude Model',
    select_claude_model: 'Select which Claude model to use for generation',
    claude_sonnet_45: 'Claude Sonnet 4.5 (Recommended)',
    claude_haiku_45: 'Claude Haiku 4.5 (Fast & Cheap)',
    claude_opus_45: 'Claude Opus 4.5 (Most Capable)',
    claude_sonnet_40: 'Claude Sonnet 4 (Legacy)',
    choose_gemini_model: 'Choose Gemini Model',
    select_gemini_model: 'Select which Gemini model to use for generation',
    gemini_25_flash: 'Gemini 2.5 Flash (Recommended)',
    gemini_25_pro: 'Gemini 2.5 Pro (Most Capable)',
    gemini_25_flash_lite: 'Gemini 2.5 Flash Lite (Fast & Cheap)',
    gemini_3_flash_preview: 'Gemini 3 Flash (Preview - Latest)',
    gemini_3_pro_preview: 'Gemini 3 Pro (Preview - Most Advanced)',
    loading_models_placeholder: 'Loading models...',
    api_key: 'API Key',
    api_key_stored_locally: 'API key stored locally',
    generate_questions_btn: 'Generate Questions',
    supported_formats: 'Supported Formats',
    or_upload_file: 'Or upload file',
    type_message: 'Type a message...',
    paste_content_here: 'Paste your text content here, or upload a file below...',
    send_message: 'Send message',
    content_type_detected: 'Content type detected',

    // AI Content Analysis
    content_analysis: 'Content Analysis',
    detected_type: 'Type:',
    formatting: 'Formatting:',
    content_mode: 'Mode:',
    detected_language: 'Language:',
    cognitive_level: "Cognitive Level (Bloom's):",
    estimated_cost: 'Estimated:',
    blooms_mixed: 'Mixed Levels',
    blooms_remember: 'Remember (Recall facts)',
    blooms_understand: 'Understand (Explain concepts)',
    blooms_apply: 'Apply (Use in new situations)',
    blooms_analyze: 'Analyze (Break down, compare)',
    blooms_evaluate: 'Evaluate (Judge, critique)',
    blooms_create: 'Create (Design, construct)',

    // AI Content Types
    content_type_mathematics: 'Mathematics',
    content_type_programming: 'Programming',
    content_type_physics: 'Physics',
    content_type_chemistry: 'Chemistry',
    content_type_biology: 'Biology',
    content_type_history: 'History',
    content_type_economics: 'Economics',
    content_type_general: 'General',

    // AI Formatting Types
    format_latex: 'LaTeX math',
    format_code: 'Code blocks',
    format_standard: 'Standard',

    // AI Generation Mode
    mode_format_existing: 'Format existing',
    mode_generate_new: 'Generate new',

    // AI Recommendations
    recommendation_existing_questions: 'Existing questions detected. The AI will format and structure them.',
    recommendation_math_content: 'Math content detected. Questions will include LaTeX formatting.',
    recommendation_code_content: 'Code detected. Questions will include syntax-highlighted code blocks.',
    recommendation_rich_content: 'Rich content detected. Consider generating multiple questions.',

    // Cost display
    cost_free: 'Free',

    // Language indicator
    questions_generated_in: 'Questions will be generated in:',

    // PDF extraction
    extracting_pdf: 'Extracting text from PDF...',
    pdf_extracted: 'PDF extracted: {pages} pages',
    pdf_no_text: 'No text content found in PDF',
    pdf_extraction_failed: 'PDF extraction failed',

    // DOCX extraction
    extracting_docx: 'Extracting text from Word document...',
    docx_extracted: 'Word document extracted',
    docx_no_text: 'No text content found in document',

    // PPTX extraction
    extracting_pptx: 'Extracting text from slides...',
    pptx_extracted: 'Slides extracted: {count} slides',
    pptx_no_text: 'No text content found in presentation',

    // URL extraction
    enter_url: 'Or enter a URL:',
    fetch_content: 'Fetch',
    url_hint: 'Web pages will be converted to text',
    url_placeholder: 'https://...',
    fetching_url: 'Fetching content...',
    url_fetched: 'Content extracted from URL',
    url_blocked: 'This URL cannot be accessed',
    url_no_text: 'No text content found at URL',
    url_minimal_text: 'Only {count} words extracted - page may use JavaScript rendering',
    invalid_url: 'Please enter a valid URL',
    words: 'words',

    // Question preview modal
    preview_generated_questions: 'Preview Generated Questions',
    preview_instructions: 'Review the generated questions below. Click to select which ones to add to your quiz.',
    select_all: 'Select All',
    deselect_all: 'Deselect All',
    selected_questions: 'Selected:',
    questions_label: 'questions',
    add_selected_questions: 'Add Selected Questions',
    no_questions_selected: 'No questions selected',
    select: 'Select',
    deselect: 'Deselect',
    question_type_multiple_choice: 'Multiple Choice',
    question_type_true_false: 'True/False',
    question_type_multiple_correct: 'Multiple Correct',
    question_type_numeric: 'Numeric',
    question_type_ordering: 'Ordering',

    load_saved_quiz: 'Load Saved Quiz',
    total: 'Total',
    created: 'Created',
    question_load_error: 'Unable to load question',

    // Results management keys
    quiz_results: 'Quiz Results',
    search_results: 'Search by quiz title or PIN...',
    sort_results: 'Sort results',
    all_dates: 'All Dates',
    today: 'Today',
    this_week: 'This Week',
    this_month: 'This Month',
    loading_results: 'Loading results...',
    no_results_found: 'No Results Found',
    no_results_message: 'No quiz results match your criteria. Try adjusting your search or date filter.',
    participants: 'Participants',
    date_completed: 'Date',
    actions: 'Actions',
    result_preview: 'Result Preview',
    duration: 'Duration',
    average_score: 'Average Score',
    player_results: 'Player Results',
    rank: 'Rank',
    player_name: 'Player Name',
    score: 'Score',
    answers: 'Answers',
    export_csv: 'Export CSV',
    export_json: 'Export JSON',
    view_results_tooltip: 'View quiz results',
    failed_fetch_results: 'Failed to fetch results',
    failed_fetch_result: 'Failed to fetch result',
    result_exported_successfully: 'Result exported successfully',
    failed_export_result: 'Failed to export result',
    failed_show_results: 'Failed to show results',
    failed_preview_result: 'Failed to preview result',

    // Simple download tool keys
    download_csv: 'Download CSV',
    download_results: 'Download Results',
    select_results: 'Select results to download...',
    finish_quiz: 'Finish Quiz',

    // Results Viewer keys
    view_quiz_results: 'View Quiz Results',
    total_quizzes: 'Total Quizzes',
    total_participants: 'Total Participants',
    avg_score: 'Average Score',
    search_quiz_title: 'Search by quiz title...',
    sort_newest_first: 'Newest First',
    sort_oldest_first: 'Oldest First',
    sort_title_az: 'Title A-Z',
    sort_most_participants: 'Most Participants',
    delete_result: 'Delete Result',

    // Game UI text
    question_will_appear_here: 'Question will appear here',
    question_alternatives: 'Question Alternatives',
    players_have_answered: 'players have answered',

    // Connection quality keys
    connection_excellent: 'Excellent',
    connection_good: 'Good',
    connection_fair: 'Fair',
    connection_poor: 'Poor',
    connection: 'Connection',

    // Question type display
    numeric_answer: 'Numeric Answer',

    // Image upload error messages
    please_select_image: 'Please select an image file',
    image_too_large: 'Image file is too large. Please select a file smaller than 5MB.',
    image_upload_failed: 'Failed to upload image. Please try again.',
    results_viewer_failed: 'Failed to load results viewer. Please try again.',

    // Confirmation dialogs
    confirm_delete_result: 'Are you sure you want to delete this quiz result? This action cannot be undone.',
    analytics_export_coming_soon: 'Analytics report export coming soon!',

    // Missing preview translations
    no_questions_to_preview: 'No questions to preview',

    // Mobile preview showcase
    see_it_in_action: 'See it in Action',
    preview_description: 'Experience interactive questions with math equations, images, and multiple choice options',

    // Mobile editor menu items
    to_top: 'To Top',

    // Accessibility labels
    previous_slide: 'Previous',
    next_slide: 'Next',
    go_to_slide: 'Go to slide',
    editing_tools: 'Editing Tools',
    connected_players: 'Connected Players',
    game_info_controls: 'Game Information and Controls',
    click_to_copy_pin: 'Click to copy PIN',
    start_quiz_game: 'Start the quiz game',

    // Additional missing keys
    copied: 'Copied!',
    error_loading_results: 'Error loading results',
    untitled_quiz: 'Untitled Quiz',
    quiz_results_detail: 'Quiz Results Detail',
    unknown: 'Unknown',
    loading: 'Loading...',
    view_all_results: 'View All Results',

    // Sound settings
    toggle_sound_tooltip: 'Toggle sound',
    sound: 'Sound',
    mute_sound: 'Mute sound',
    unmute_sound: 'Unmute sound',
    // Host "mute all students" control
    mute_all_students: 'Mute students',
    unmute_all_students: 'Unmute students',
    muted_by_host: 'Muted by your teacher',

    // Onboarding tutorial (v2 - rewritten)
    onboarding_step1_title: 'Create a Quiz',
    onboarding_step1_content: 'Click here to enter the quiz editor and start creating your own interactive quiz.',
    onboarding_step2_title: 'Name Your Quiz',
    onboarding_step2_content: 'Give your quiz a memorable title. This is what players will see when they join.',
    onboarding_step3_title: 'Question Types',
    onboarding_step3_content: 'The live preview on the right shows how players see your quiz. Use this dropdown to choose from multiple choice, true/false, multiple correct, numeric, or ordering questions.',
    onboarding_step4_title: 'Navigate Questions',
    onboarding_step4_content: 'Use the arrows to move between questions. Each question is shown one at a time for focused editing.',
    onboarding_step5_title: 'AI Generator',
    onboarding_step5_content: 'Let AI create questions for you! Paste any text, article, or topic and get instant quiz questions.',
    onboarding_step6_title: 'Save Your Work',
    onboarding_step6_content: 'This sidebar has all your tools — save, load, import, export, and settings. Save your quiz to continue editing later.',
    onboarding_step7_title: 'Start the Game',
    onboarding_step7_content: 'Ready to play? Create a lobby and share the PIN code with your players.',
    onboarding_step8_title: 'Join a Game',
    onboarding_step8_content: 'Want to play instead? Enter the 6-digit PIN from a host to join their quiz.',
    onboarding_next: 'Next',
    onboarding_back: 'Back',
    onboarding_skip: 'Skip Tour',
    onboarding_finish: 'Get Started!',
    onboarding_dont_show: "Don't show this again",
    onboarding_step_of: 'Step {0} of {1}',

    // AI Generator error messages
    ollama_not_running: 'Ollama server not running. Please start Ollama and try again.',
    ollama_connection_error: 'Cannot connect to Ollama. Please ensure the Ollama server is configured and running.',
    ollama_not_available: 'Ollama not available',
    ollama_loading_models: 'Loading models...',
    ollama_no_models: 'No models found',
    invalid_api_key: 'Invalid API key. Please check your credentials.',
    rate_limit_exceeded: 'Rate limit exceeded. Please try again later.',
    billing_issue: 'Billing issue. Please check your account balance and payment method.',
    api_access_forbidden: 'API access forbidden. Please check your API key permissions.',
    invalid_response_structure: 'Invalid API response structure',
    content_blocked_safety: 'Content blocked by safety filters. Try rephrasing your prompt.',
    code_model_warning: 'Code models are designed for code generation, not quiz creation. Please use a general model instead.',
    no_provider_selected: 'No AI Provider Selected',
    no_provider_selected_msg: 'Please select an AI provider before generating questions.',
    no_content_provided: 'No Content Provided',
    no_content_provided_msg: 'Please enter source content to generate questions from.',
    no_question_types: 'No Question Types Selected',
    no_question_types_msg: 'Please select at least one question type.',
    generation_failed: 'Generation Failed',
    no_questions_generated: 'No Questions Generated',
    retry_attempt: 'Retry attempt {0} of {1}...',
    processing_batch: 'Processing batch {0} of {1}...',

    // Server error messages
    pdf_size_limit: 'PDF file exceeds 10MB limit. Please use a smaller file or copy text manually.',
    pdf_parsing_unavailable: 'Server does not have PDF parsing capability. Please copy and paste the text content manually.',
    too_many_requests: 'Too many requests. Please wait {0} seconds.',
    invalid_quiz_data: 'Invalid quiz data',
    quiz_needs_questions: 'Quiz must have at least one question',
    invalid_request_data: 'Invalid request data',
    failed_create_game: 'Failed to create game',
    failed_join_game: 'Failed to join game',
    failed_start_game: 'Failed to start game',
    invalid_pin_format: 'Invalid PIN format. Must be 6 digits.',

    // Results viewer messages
    failed_load_results: 'Failed to load quiz results',
    no_analytics_data: 'No player response data available for analytics.',
    analytics_generation_failed: 'Failed to generate analytics. Please check the console for details.',
    select_result_download: 'Please select a result to download',
    download_failed: 'Download failed. Please try again.',
    results_viewer_error: 'Could not open detailed results viewer',

    // Application error
    app_init_error: 'Application Error',
    app_init_error_msg: 'Failed to initialize Quizix Pro. Please refresh the page.',

    // Practice Mode
    practice_mode: 'Practice Mode',
    practice: 'Practice',
    practice_complete: 'Practice Complete!',
    accuracy: 'Accuracy',
    time: 'Time',
    new_personal_best: 'New Personal Best!',
    personal_best: 'Personal Best',
    try_again: 'Try Again',
    exit: 'Exit',
    failed_start_practice: 'Failed to start practice mode',
    confirm_exit_practice: 'Exit practice and return to home?',
    exit_to_home: 'Exit to Home',
    load: 'Load',

    // Power-Ups
    enable_power_ups: 'Enable Power-Ups (50-50, +10s, 2x Points)',
    enable_power_ups_tooltip: 'Allow players to use power-ups during the game',
    show_leaderboard_between: 'Show leaderboard after each question',
    show_leaderboard_between_tooltip: 'Show the standings between questions; the final leaderboard always appears',
    power_ups: 'Power-Ups',
    fifty_fifty: '50-50',
    fifty_fifty_tooltip: 'Remove half of the wrong answers',
    extend_time: '+10s',
    extend_time_tooltip: 'Add 10 seconds to the timer',
    double_points: '2× Points',
    double_points_tooltip: 'Double your points on the next correct answer',
    power_up_used: 'Power-up used!',

    // Scoring Settings
    scoring_settings: '📊 Scoring Settings',
    enable_time_bonus: 'Enable time bonus',
    time_bonus_help: 'When ON, faster answers earn more points (up to +1000 for easy, +2000 medium, +3000 hard). The bonus decreases proportionally until the question timer runs out — answering at the last second still earns a small bonus above base. When OFF, all correct answers earn flat base points regardless of speed.',
    show_score_breakdown: 'Show score breakdown (host view)',
    difficulty_multipliers: 'Difficulty Multipliers:',
    difficulty_multipliers_help: 'Base points per correct answer = 100 × multiplier. E.g. Easy ×1 = 100 pts, Medium ×2 = 200 pts, Hard ×3 = 300 pts. Time bonus also scales with the multiplier.',
    scoring: 'Scoring:',
    base: 'Base',
    time_bonus: 'Time bonus',
    enabled: 'ON',
    disabled: 'OFF',
    time_bonus_threshold: 'Equal-points window:',
    time_bonus_threshold_help: 'If set above 0, all players who answer within this many seconds get full bonus — no speed advantage. After this window, the bonus decreases linearly. Set to 0 to reward speed from the start.',
    seconds_short: 'sec',
    max_within: 'max within',

    // File Management
    new_folder: 'New Folder',
    rename: 'Rename',
    move_to: 'Move To...',
    delete: 'Delete',
    expand_all: 'Expand All',
    collapse_all: 'Collapse All',
    enter_folder_name: 'Enter folder name:',
    enter_new_name: 'Enter new name:',
    enter_number: 'Enter number (0 for root):',
    invalid_selection: 'Invalid selection',
    folder_created: 'Folder created',
    renamed_successfully: 'Renamed successfully',
    moved_successfully: 'Moved successfully',
    deleted_successfully: 'Deleted successfully',
    confirm_delete_folder: 'Delete folder "{name}" and all its contents?',
    confirm_delete_quiz: 'Delete quiz "{name}"?',
    empty_folder: '(empty)',
    no_quizzes_yet: 'No quizzes yet',

    // Password Protection
    set_password: 'Set Password',
    remove_password: 'Remove Password',
    enter_password: 'Enter Password',
    password: 'Password',
    new_password: 'New Password',
    confirm_password: 'Confirm Password',
    unlock: 'Unlock',
    password_protected: 'Password protected',
    password_required_for: 'Password required for',
    set_password_for: 'Set password for',
    password_set: 'Password set successfully',
    password_removed: 'Password removed',
    confirm_remove_password: 'Remove password protection?',
    password_required: 'Password is required',
    password_too_short: 'Password must be at least 4 characters',
    passwords_dont_match: 'Passwords do not match',
    incorrect_password: 'Incorrect password',
    too_many_attempts: 'Too many attempts. Please try again later.',
    password_weak: 'Weak',
    password_fair: 'Fair',
    password_good: 'Good',
    password_strong: 'Strong',
    optional_password: 'Password (optional)',
    password_hint: 'If set, password will be required to load or delete this quiz',
    password_placeholder: 'Enter password...',
    confirm_password_placeholder: 'Confirm password...',
    verify_to_remove_password: 'Enter current password to remove protection',
    right_click_hint: 'Right-click (or long-press on mobile) for more options',

    // Quiz Settings Modal
    quiz_settings: 'Quiz Settings',
    quiz_settings_tooltip: 'Open quiz settings',
    general_options: 'General Options',
    advanced_options: 'Advanced Options',
    editor_mode_basic: 'Basic',
    editor_mode_advanced: 'Advanced',

    // Missing keys found during audit
    answer_not_submitted: 'Answer could not be submitted',
    correct: 'Correct',
    enter_fullscreen: 'Enter fullscreen',
    exit_fullscreen: 'Exit fullscreen',
    enter_your_answer: 'Enter your answer',
    error: 'Error',
    failed_generate_qr_code: 'Failed to generate QR code',
    failed_load_quizzes: 'Failed to load quizzes',
    false_display: 'False',
    incorrect_answer: 'Incorrect!',
    info: 'Info',
    invalid_quiz_questions: 'Invalid quiz questions',
    no_saved_quizzes: 'No saved quizzes found',
    player: 'Player',
    success: 'Success',
    switch_dark_mode: 'Switch to dark mode',
    switch_light_mode: 'Switch to light mode',
    true_display: 'True',
    type: 'Type',

    // Concept tags
    concepts: 'Concepts',
    add_concept: 'Add concept...',
    concept_hint: 'Press Enter to add (max 5)',

    // Consensus Mode
    consensus_mode: 'Consensus Mode',
    consensus_mode_tooltip: 'Team collaboration mode where players vote together',
    consensus_threshold: 'Consensus Threshold',
    threshold_50: '50% - Simple majority',
    threshold_66: '66% - Two-thirds majority',
    threshold_75: '75% - Strong consensus',
    threshold_100: '100% - Unanimous',
    discussion_time: 'Discussion Time',
    allow_chat: 'Allow free-form chat',
    allow_chat_hint: 'Players can type messages in addition to quick responses',
    team_score: 'Team Score',
    team_points: 'team points',
    team_achievement: 'Team Achievement',
    propose_answer: 'Propose Answer',
    consensus_reached: 'Consensus Reached!',
    consensus_correct: 'Correct!',
    consensus_incorrect: 'Incorrect',
    waiting_consensus: 'Waiting for consensus...',
    lock_consensus: 'Lock Consensus',
    lock_consensus_tooltip: 'Lock in the current consensus answer',
    proposals: 'Proposals',
    discussion: 'Discussion',
    quick_responses: 'Quick Responses',

    // Quick response types
    i_think_its: 'I think it\'s...',
    i_agree: 'I agree',
    i_agree_with: 'I agree with {player}',
    not_sure: 'Not sure',
    lets_discuss: 'Let\'s discuss',
    ready_to_lock: 'Ready to lock in',

    // Consensus progress
    consensus_progress: 'Consensus Progress',
    threshold_needed: 'Threshold needed',
    current_consensus: 'Current consensus',

    // Manim Editor & Tutorial
    manim_enter_code_first: 'Please enter Manim code first',
    manim_rendering: 'Rendering animation...',
    manim_rendered_successfully: 'Animation rendered successfully!',
    manim_render_failed: 'Animation render failed',
    manim_question_animation_tab: 'Question Animation',
    manim_explanation_animation_tab: 'Explanation Animation',
    manim_code_label: 'Manim Code',
    render_animation: 'Render Animation',
    remove_video: 'Remove Video',
    manim_tutorial_title: 'Manim Animation Tutorial',
    manim_tutorial_close: 'Close Tutorial',
    manim_what_are_animations: 'What are Animations?',
    manim_what_are_animations_desc: 'Manim animations are short mathematical videos that bring your quiz questions to life. They can visualize geometric concepts, show step-by-step solutions, or illustrate complex mathematical ideas.',
    manim_how_to_use: 'How to Use',
    manim_step_question_tab: 'Switch to the "Question Animation" or "Explanation Animation" tab',
    manim_step_explanation_tab: 'Each question can have both a question and explanation animation',
    manim_step_write_code: 'Write your Manim code in the editor',
    manim_step_click_render: 'Click "Render Animation" to preview',
    manim_step_preview_save: 'Preview and save with your question',
    manim_code_examples: 'Code Examples',
    manim_code_examples_desc: 'Here are some starter templates you can copy and modify:',
    manim_example_basic_shapes: 'Basic Shapes',
    manim_example_text: 'Text & Math',
    manim_example_number_line: 'Number Line',
    manim_example_graph: 'Function Graph',
    manim_tips: 'Tips & Best Practices',
    manim_tip_keep_short: 'Keep animations under 10 seconds for best engagement',
    manim_tip_use_wait: 'Use self.wait() to add pauses between animations',
    manim_tip_scene_class: 'Your class must inherit from Scene',
    manim_tip_import: 'No need to add imports - they are included automatically',
    manim_tip_mathtex: 'Use Text() for labels — MathTex() requires LaTeX installed',
    manim_tutorial_help_tooltip: 'Open Manim animation tutorial',
    manim_copy_code_aria: 'Copy code to clipboard',
    manim_ai_describe: 'Describe your animation:',
    manim_ai_placeholder: 'e.g., Show a circle with radius 2 being drawn, then display its area formula',
    manim_ai_generate: 'Generate with AI',
    manim_ai_regenerate: 'Regenerate',
    manim_ai_settings: 'AI Settings',
    manim_ai_provider: 'Provider:',
    manim_ai_model: 'Model:',
    manim_ai_key_shared: '(shared with question generator)',
    manim_ai_enter_description: 'Please enter an animation description first',
    manim_ai_generating: 'Generating code...',
    manim_ai_generated: 'Code generated successfully!',
    manim_ai_failed: 'Generation failed',

    // Results/Analytics
    results_failed_load: 'Failed to load results',
    results_downloaded: 'Results downloaded successfully',
    results_not_available: 'Results not available',
    results_refreshed: 'Results refreshed',
    results_failed_refresh: 'Failed to refresh results',
    results_failed_delete: 'Failed to delete result',
    results_failed_analytics: 'Failed to load analytics',
    results_no_player_data: 'No player response data available',
    results_failed_generate_analytics: 'Failed to generate analytics',
    results_not_found: 'Result not found',
    failed_export_pdf: 'Failed to export PDF',
    failed_export_excel: 'Failed to export Excel file',
    results_select_format: 'Select Format',
    results_choose_format: 'Choose export format',
    analytics_not_available: 'Advanced Analytics Not Available',
    analytics_not_available_title: 'Advanced Analytics Not Available',
    analytics_not_available_desc: 'Detailed question-level analytics require individual player response data, which was not recorded for this game session.',
    analytics_still_see: 'You can still see:',
    analytics_player_scores: 'Player scores and rankings',
    analytics_completion_data: 'Completion data',
    analytics_participation_stats: 'Participation statistics',
    analytics_performance_summary: 'Performance summary',
    analytics_why_not_available: 'Why is this not available?',
    analytics_why_not_available_desc: 'Per-question analytics are only available when the game records individual answer data for each player on each question.',
    analytics_future_games: 'For Future Games',
    analytics_future_games_desc: 'Analytics are automatically available for all new games played through the lobby system.',
    analytics_view_basic: 'View Basic Results',
    analytics_quiz_label: 'Quiz',
    analytics_pin_label: 'PIN',
    analytics_participants_label: 'Participants',
    analytics_overview_tab: 'Overview',
    analytics_questions_tab: 'Questions',
    analytics_insights_tab: 'Insights',
    analytics_concepts_tab: 'Concepts',
    analytics_avg_success_rate: 'Avg. Success Rate',
    analytics_avg_response_time: 'Avg. Response Time',
    analytics_needs_review: 'Needs Review',
    analytics_click_question: 'Click a question to see detailed analysis',
    analytics_avg: 'avg',
    analytics_responses: 'responses',
    analytics_avg_points: 'avg points',
    analytics_content_review: 'Content Review',
    analytics_no_issues: 'No issues detected',
    analytics_performance_insights: 'Performance Insights',
    analytics_hardest_question: 'Hardest Question',
    analytics_easiest_question: 'Easiest Question',
    analytics_success_rate: 'success rate',
    analytics_needs_review_alert: 'Questions Needing Review',
    analytics_concept_mastery: 'Concept Mastery',
    analytics_all_concepts_well: 'All concepts performing well',
    analytics_students: 'students',
    analytics_all_correct: 'All correct',
    analytics_question_details: 'Question Details',
    analytics_answer_distribution: 'Answer Distribution',
    analytics_response_time_dist: 'Response Time Distribution',
    analytics_common_wrong: 'Most Common Wrong Answer',
    analytics_issues_detected: 'Issues Detected',
    analytics_correct: 'Correct',
    analytics_incorrect: 'Incorrect',
    analytics_avg_time: 'Avg. Time',
    no_answer: 'No answer',
    analytics_correct_answer: 'Correct answer',
    analytics_unanswered: 'Not reached',
    analytics_unanswered_note: 'No one answered this question',
    analytics_flag_low_success: 'Only {0}% success rate - potential knowledge gap',
    analytics_flag_moderate_success: '{0}% success rate - room for improvement',
    analytics_flag_slow_and_wrong: 'Slow ({0}s) and mostly wrong - conceptual difficulty',
    analytics_flag_quick_wrong: 'Quick responses with errors - potential misconceptions',
    analytics_flag_common_wrong: '{0} students chose "{1}" - potentially misleading option',
    analytics_concepts_need_work: '{0} concept(s) need improvement',
    analytics_concepts_mastered: '{0} concept(s) mastered',
    analytics_dependency_suggestion: 'Strengthen "{0}" to improve "{1}"',
    export_csv_btn: 'Export CSV',
    export_json_btn: 'Export JSON',
    export_pdf_btn: 'Export PDF',
    export_excel_btn: 'Export Excel',
    download_btn: 'Download',
    pts: 'pts',

    // Session Comparison
    compare_sessions: 'Compare Sessions',
    compare_sessions_tooltip: 'Compare results across multiple game sessions',
    compare_no_multi_sessions: 'Need at least 2 sessions of the same quiz to compare',
    compare_quiz_sessions: 'Quiz Session Comparison',
    compare_select_quiz_desc: 'Select a quiz to compare its sessions',
    compare_sessions_count: 'sessions',
    compare_total_participants: 'total participants',
    compare_btn: 'Compare',
    compare_select_sessions: 'Select Sessions to Compare',
    compare_select_sessions_desc: 'Choose 2-5 sessions to analyze trends',
    compare_cancel: 'Cancel',
    compare_selected: 'Compare Selected',
    compare_min_sessions: 'Please select at least 2 sessions',
    compare_max_sessions: 'Maximum 5 sessions can be compared',
    compare_not_enough_data: 'Not enough data to generate comparison',
    compare_failed_generate: 'Failed to generate comparison',
    compare_failed_data: 'Failed to load comparison data',
    compare_most_improved: 'Most Improved',
    compare_needs_attention: 'Needs Attention',
    compare_session_label: 'Session',
    compare_avg_participants: 'Avg. Participants',
    compare_key_insights: 'Key Insights',
    compare_stable_performance: 'Stable performance across sessions',
    compare_improving: 'Improving',
    compare_declining: 'Declining',
    compare_stable: 'Stable',
    compare_session_details: 'Session Details',
    compare_question_trends: 'Question Trends',
    compare_report_downloaded: 'Comparison report downloaded',
    compare_failed_report: 'Failed to download comparison report',

    // AI Generator Additional
    ai_enter_api_key: 'Enter your API key',
    ai_loading_models: 'Loading available models...',
    ai_no_models_found: 'No models found',
    ai_ollama_not_available: 'Ollama is not available',
    ai_model_size_gb: '{size} GB',
    ai_model_fallback: 'Model not available, using fallback',
    ai_enter_content_placeholder: 'Paste your content here (text, article, notes...)',
    ai_retry_attempt: 'Retry attempt {0} of {1}',
    ai_tokens_display: '{0} tokens',
    ai_excel_not_available: 'Excel processing is not available',
    ai_failed_process_excel: 'Failed to process Excel file',
    ai_excel_batch_info: 'Processing {0} questions in {1} batches',
    ai_batch_failed: 'Batch {0} failed',
    ai_all_batches_complete: 'All batches processed',
    ai_batch_no_questions: 'No questions generated from batch',
    ai_question_updated: 'Question updated',
    ai_question_regenerated: 'Question regenerated successfully',
    ai_failed_regenerate: 'Failed to regenerate question',
    ai_recommended: 'Recommended',
    ai_fast_cheap: 'Fast & Cheap',
    ai_most_capable: 'Most Capable',
    ai_legacy: 'Legacy',
    ai_preview_latest: 'Preview (Latest)',

    // Socket/Server Error Messages
    error_occurred: 'An error occurred',
    error_game_not_found: 'Game not found',
    error_player_limit: 'Player limit reached',
    error_invalid_pin: 'Invalid game PIN',
    error_name_taken: 'Name is already taken',
    error_game_ended: 'Game has ended',
    error_disconnected: 'Disconnected from server',
    error_reconnect_failed: 'Failed to reconnect',
    reconnecting: 'Reconnecting...',
    reconnected_successfully: 'Reconnected!',
    rejoin_failed: 'Could not rejoin the game',
    error_invalid_quiz_data: 'Invalid quiz data',
    error_quiz_needs_questions: 'Quiz must have at least one question',
    error_host_new_game: 'Please host a new game',
    error_failed_create_game: 'Failed to create game',
    error_failed_start_game: 'Failed to start game',
    error_can_only_rematch: 'Can only rematch from a finished game',
    error_failed_rematch: 'Failed to create rematch',
    error_invalid_request: 'Invalid request',
    error_failed_join: 'Failed to join game',
    error_player_not_found: 'Player not found',
    error_failed_change_name: 'Failed to change name',
    error_invalid_answer: 'Invalid answer',
    error_session_not_found: 'Session not found',
    error_server_error: 'Server error',
    error_invalid_powerup: 'Invalid power-up',
    error_powerup_not_enabled: 'Power-ups are not enabled',
    error_powerup_already_used: 'Power-up already used',
    error_unknown_powerup: 'Unknown power-up type',
    error_game_full: 'Game is full',
    error_game_already_started: 'Game has already started',
    error_answer_rejected: 'Answer was rejected',
    error_pin_name_required: 'PIN and name are required',
    error_name_length: 'Name must be between 1 and 20 characters',
    error_name_invalid_chars: 'Name contains invalid characters',
    error_host_disconnected: 'Host disconnected',
    error_name_required: 'Name is required',
    error_name_change_started: 'Cannot change name after game has started',
    error_name_already_taken: 'Name is already taken in this game',
    error_rate_limited: 'Too many requests. Please slow down.',

    // Consensus Server Messages
    consensus_not_active: 'Consensus mode is not active',
    consensus_no_question: 'No active question',
    consensus_already_locked: 'Answer is already locked',
    consensus_invalid_answer: 'Invalid answer option',
    consensus_failed_propose: 'Failed to propose answer',
    consensus_invalid_response: 'Invalid quick response',
    consensus_chat_disabled: 'Chat is disabled for this game',
    consensus_empty_message: 'Message cannot be empty',
    consensus_failed_chat: 'Failed to send message',
    consensus_host_only_lock: 'Only the host can lock the answer',
    consensus_failed_lock: 'Failed to lock answer',
    consensus_qr_i_think: 'I think it\'s...',
    consensus_qr_i_agree: 'I agree',
    consensus_qr_not_sure: 'Not sure',
    consensus_qr_lets_discuss: 'Let\'s discuss',
    consensus_qr_ready_lock: 'Ready to lock in',

    // HTTP API Errors
    error_loading_app: 'Error loading application',
    error_cross_origin: 'Cross-origin request blocked',
    error_delete_requires_confirm: 'Delete requires confirmation',
    error_invalid_filename: 'Invalid filename',
    error_auth_required: 'Authentication required',
    error_auth_invalid: 'Invalid authentication',
    error_only_images: 'Only image files are allowed',
    error_no_file: 'No file provided',
    error_file_not_saved: 'File could not be saved',
    error_empty_file: 'File is empty',
    error_invalid_image: 'Invalid image file',
    error_upload_failed: 'Upload failed',
    error_only_pdf: 'Only PDF files are allowed',
    error_no_pdf: 'No PDF file provided',
    error_pdf_too_large: 'PDF file is too large',
    error_pdf_too_large_desc: 'PDF file exceeds the 10MB limit. Please use a smaller file.',
    error_pdf_not_available: 'PDF processing not available',
    error_pdf_not_available_desc: 'Server does not have PDF parsing capability.',
    error_pdf_timeout: 'PDF processing timed out',
    error_pdf_failed: 'PDF processing failed',
    error_no_docx: 'No Word document provided',
    error_docx_not_available: 'Word document processing not available',
    error_docx_timeout: 'Word document processing timed out',
    error_docx_failed: 'Word document processing failed',
    error_no_pptx: 'No presentation file provided',
    error_pptx_not_available: 'Presentation processing not available',
    error_pptx_timeout: 'Presentation processing timed out',
    error_pptx_failed: 'Presentation processing failed',
    error_invalid_url_format: 'Invalid URL format',
    error_url_blocked: 'URL is blocked',
    error_url_blocked_desc: 'This URL cannot be accessed for security reasons.',
    error_url_not_available: 'URL fetching not available',
    error_url_fetch_failed: 'Failed to fetch URL content',
    error_url_unsupported_type: 'Unsupported content type',
    error_url_timeout: 'URL fetch timed out',
    error_url_failed: 'URL fetch failed',
    error_api_key_required: 'API key is required',
    error_api_key_invalid: 'Invalid API key',
    error_rate_limit: 'Rate limit exceeded',
    error_invalid_request_check: 'Invalid request. Please check your input.',
    error_access_forbidden: 'Access forbidden',
    error_quota_exceeded: 'API quota exceeded',
    error_api_connect_failed: 'Failed to connect to API',
    error_ollama_fetch_failed: 'Failed to fetch Ollama models',
    error_ollama_connect_failed: 'Cannot connect to Ollama server',
    error_manim_code_required: 'Manim code is required',
    error_manim_quality_invalid: 'Invalid quality setting',
    error_manim_rate_limit: 'Too many render requests. Please wait.',
    error_manim_timeout: 'Animation render timed out',
    error_manim_render_failed: 'Animation render failed',
    error_manim_status_failed: 'Failed to check render status',

    // Service Validation Messages
    error_quiz_title_too_long: 'Quiz title is too long',
    error_quiz_max_questions: 'Maximum number of questions exceeded',
    error_question_text_too_long: 'Question text is too long',
    error_question_explanation_too_long: 'Explanation is too long',
    error_option_too_long: 'Answer option is too long',
    error_quiz_not_found: 'Quiz not found',
    error_result_not_found: 'Result not found',
    error_result_corrupted: 'Result data is corrupted',
    error_unsupported_format: 'Unsupported format',
    error_invalid_export_type: 'Invalid export type',
    error_directory_traversal: 'Invalid path',
    error_invalid_results: 'Invalid results data',
    error_folder_not_found: 'Folder not found',
    error_folder_exists: 'Folder already exists',
    error_target_folder_not_found: 'Target folder not found',
    error_move_into_self: 'Cannot move folder into itself',
    error_move_into_descendant: 'Cannot move folder into its own subfolder',
    error_folder_not_empty: 'Folder is not empty',
    error_folder_name_required: 'Folder name is required',
    error_folder_name_empty: 'Folder name cannot be empty',
    error_folder_name_too_long: 'Folder name is too long',
    error_folder_name_invalid: 'Folder name contains invalid characters',
    error_quiz_not_in_metadata: 'Quiz not found in metadata',
    error_display_name_required: 'Display name is required',
    error_display_name_too_long: 'Display name is too long',
    error_item_not_protected: 'Item is not password protected',
    error_validation_failed: 'Validation failed',
    error_invalid_parameters: 'Invalid parameters',
    error_invalid_query: 'Invalid query parameters',
    error_game_limit_reached: 'Game limit reached',
    error_manim_disabled: 'Manim rendering is disabled',
    error_manim_no_scene: 'No Scene class found in code',
    error_manim_no_output: 'No output file generated',

    // Misc UI
    menu: 'Menu',
    toggle_theme: 'Toggle Theme',
    fullscreen: 'Fullscreen',
    show_ui: 'Show UI',
    loading_default: 'Loading...',
    no_question_text: 'No question text',
    image_failed_load: 'Image failed to load',
    image_check_exists: 'Please check that the image file exists',
    enter_numeric_placeholder: 'Enter a number...',
    no_ordering_options: 'No ordering options available',
    resize_split_view: 'Resize split view',
    pin_copied: 'PIN copied to clipboard!',
    pin_label: 'PIN:',
    add_question_title: 'Add Question',
    previous_question: 'Previous Question',
    next_question_btn: 'Next Question',
    no_questions_placeholder: 'No questions yet. Add your first question!',
    go_to_step: 'Go to step {0}',
    click_connection_details: 'Click for connection details',
    application_error: 'Application Error',
    app_failed_init: 'Failed to initialize application',
    no_questions_yet: 'No questions yet. Click + to add one!',
    image_not_found: 'Image not found',
    image_remove_or_upload: 'Remove reference or upload file',
    result_display_error: 'Result display error',
    loading_quiz_results: 'Loading quiz results...',
    loading_version: 'Loading version...',
    multiple_type_badge: 'Multiple Correct',
    question_animation: 'Question Animation',
    explanation_animation: 'Explanation Animation',
    generated_by_quizix: 'Generated by Quizix Pro',
    anonymous_player: 'Anonymous',
    debug_info_expand: 'Debug Information',

    // Results Exporter Labels
    export_quiz_results_report: 'Quiz Results Report',
    export_quiz_title_label: 'Quiz Title',
    export_game_pin_label: 'Game PIN',
    export_date_label: 'Date',
    export_participants_label: 'Participants',
    export_summary_stats: 'Summary Statistics',
    export_total_questions: 'Total Questions',
    export_questions_needing_review: 'Questions Needing Review',
    export_question_num: 'Question {0}',
    export_question_text: 'Question Text',
    export_type_label: 'Type',
    export_success_rate_label: 'Success Rate',
    export_avg_time_label: 'Average Time',
    export_responses_label: 'Responses',
    export_avg_points_label: 'Average Points',
    export_problematic: 'Problematic',
    export_issues: 'Issues',
    export_rank_label: 'Rank',
    export_player_label: 'Player',
    export_score_label: 'Score',
    export_correct_answers: 'Correct Answers',
    export_completed_at: 'Completed At',
    export_wrong_answer: 'Wrong Answer',
    export_count: 'Count',
    export_session_comparison_report: 'Session Comparison Report',
    export_sessions_compared: 'Sessions Compared',
    export_overall_trend: 'Overall Trend',

    // Results viewer - additional strings
    results_viewer_not_available: 'Results viewer not available',
    results_refreshed_success: 'Results refreshed successfully',
    results_failed_load_detailed: 'Failed to load detailed data for analytics. Please try again.',
    results_no_player_data_analytics: 'No player response data available for analytics.',
    results_failed_generate_analytics_detail: 'Failed to generate analytics. Please check the console for details.',
    results_failed_export_pdf: 'Failed to export PDF',
    results_failed_export_excel: 'Failed to export Excel',
    results_no_multi_sessions: 'No quizzes with multiple sessions found. Run a quiz multiple times to compare results.',
    results_select_at_least_2: 'Please select at least 2 sessions to compare',
    results_select_max_5: 'Please select no more than 5 sessions for clarity',
    results_not_enough_comparison: 'Could not load enough session data for comparison',
    results_failed_comparison_data: 'Could not generate comparison data',
    results_failed_comparison: 'Failed to generate comparison',
    results_title_suffix: 'Results',

    // Results renderer - additional strings
    results_no_results_title: 'No Results Found',
    results_no_results_message: 'No quiz results match your search criteria.',
    results_no_participants_title: 'No Participants',
    results_no_participants_message: 'No participant data available for this quiz.',
    results_participant_results_header: 'Participant Results',
    results_select_export_format: 'Select Export Format',
    results_choose_download_format: 'Choose how you\'d like to download the results:',
    results_swipe: 'swipe',

    // Analytics modal - additional strings
    analytics_quiz_title: 'Quiz Analytics',
    analytics_questions_need_review: 'Questions Need Review',
    analytics_click_hint: 'Click a question for detailed breakdown',
    analytics_content_review_title: 'Content Review Recommendations',
    analytics_no_major_issues: 'No major issues detected. All questions performing well!',
    analytics_performance_insights_title: 'Performance Insights',
    analytics_quiz_needs_review: 'Quiz needs review',
    analytics_may_need_improvement: 'may need improvement',
    analytics_question_num_details: 'Question {0} Details',
    analytics_all_answers_correct: 'All answers were correct!',

    // Analytics - concept mastery strings
    analytics_mastered_label: 'Mastered (80%+)',
    analytics_proficient_label: 'Proficient (60-79%)',
    analytics_developing_label: 'Developing (40-59%)',
    analytics_needs_work_label: 'Needs Work (<40%)',
    analytics_show_suggestions: 'Show suggestions',
    analytics_study_suggestions: 'Study Suggestions',
    analytics_focus_areas: 'Focus Areas',
    analytics_strong_areas: 'Strong Areas',
    analytics_study_suggestion: 'Study Suggestion',

    // Chart titles
    chart_success_rate_by_question: 'Success Rate by Question',
    chart_time_vs_success: 'Time vs Success Rate (Red = Problematic)',
    chart_avg_time_seconds: 'Average Time (seconds)',
    chart_success_rate_pct: 'Success Rate (%)',
    chart_numeric_answer_dist: 'Numeric Answer Distribution',
    chart_number_of_answers: 'Number of Answers',
    chart_answer_range: 'Answer Range',
    chart_option_selection_dist: 'Option Selection Distribution',
    chart_correct_option: 'Correct Option',
    chart_incorrect_option: 'Incorrect Option',
    chart_selection_rate: 'Selection Rate (%)',
    chart_success_rate_comparison: 'Success Rate Comparison Across Sessions',
    chart_session_date: 'Session Date',
    chart_overall_average: 'Overall Average',
    chart_concept_mastery_levels: 'Concept Mastery Levels',
    chart_contains_correct: '(Contains correct answer)',

    // Export - additional PDF/Excel strings
    export_most_challenging: 'Most Challenging Question',
    export_easiest_question: 'Easiest Question',
    export_question_by_question: 'Question-by-Question Analysis',
    export_question_text_not_available: 'Question text not available',
    export_more_questions_csv: '... and {0} more questions (see CSV export for full data)',
    export_page_of: 'Page {0} of {1}',
    export_excel_not_loaded: 'Excel export library not loaded. Please refresh and try again.',
    export_pdf_downloaded: 'PDF report downloaded: {0}',
    export_excel_downloaded: 'Excel report downloaded: {0}',
    export_failed_pdf: 'Failed to generate PDF report',
    export_failed_excel: 'Failed to generate Excel report',
    export_failed_analytics: 'Failed to export analytics report',
    export_failed_download: 'Failed to download result',
    export_comparison_downloaded: 'Comparison report downloaded: {0}',
    export_failed_comparison: 'Failed to generate comparison report',
    export_yes: 'Yes',
    export_no: 'No',
    export_question_analysis: 'Question Analysis',
    export_wrong_answers_sheet: 'Wrong Answers',
    export_players_sheet: 'Players',
    export_summary_sheet: 'Summary',
    export_questions_sheet: 'Questions',

    // Comparison modal strings
    compare_sessions_title: 'Sessions',
    compare_trend_direction: '{0} ({1})',

    // Comparison PDF export strings
    export_avg_participants: 'Average Participants',
    export_improving: 'Improving',
    export_declining: 'Declining',
    export_stable: 'Stable',
    export_session_details: 'Session Details',
    export_question_trends: 'Question Performance Trends',
    export_session_label: 'Session {0}',
    export_question_num_header: 'Question #',
    export_question_text_header: 'Question Text',
    help_and_tour: 'Help & Tour',
    restart_tour: 'Restart Tour',
    power_ups_help: 'Give players special abilities: 50-50 removes two wrong answers, +10s adds extra time, 2x doubles points for one question.',
    consensus_mode_help: 'Players discuss and vote together before answering. The team must reach the threshold to lock in an answer.',
    create_first_quiz: 'Create your first quiz',
    try_ai_generation: 'Try AI generation to create questions automatically',
    import_quiz_hint: 'You can also import quizzes from JSON or Excel files',
    pin_format_hint: 'Enter the 6-digit PIN shown on the host\'s screen',
    waiting_hint: 'The host will start the game when all players have joined',
    choose_your_language: 'Choose Your Language',
    // Rejoin Banner
    rejoin_game_available: 'Game in progress',
    rejoin_game_pin_label: 'PIN:',
    rejoin_button: 'Rejoin',
    rejoin_dismiss: 'Dismiss',
    rejoin_expired: 'Session expired — please join a new game',
    return_to_menu: 'Return to Menu',

    // Host reconnection messages
    host_disconnected_waiting: 'Host disconnected. Waiting for reconnection...',
    host_preparing_new_game: 'Host is preparing a new game...',
    host_reconnected: 'Host reconnected!',
    rejoin_timeout: 'Reconnection timed out. Please join a new game.',

    // Session/lobby
    players_expected: 'players expected',
    release_players: 'Release All Players',
    session_waiting_title: 'Waiting for the next game...',
    session_waiting_subtitle: 'The host will start a new game soon.',
    leave_session: 'Leave Session',

    // Validation messages
    auth_check_failed: 'Unable to verify permissions. Please try again.',
    invalid_correct_answer: 'Please select a correct answer',
    invalid_numeric_answer: 'Please enter a valid numeric answer',
    invalid_quiz_format: 'Invalid quiz file format',
    select_at_least_one_correct: 'Please select at least one correct answer',

    // Import/Export messages
    failed_export_quiz: 'Failed to export quiz',
    failed_import_quiz: 'Failed to import quiz',
    quiz_exported_successfully: 'Quiz exported successfully',
    quiz_imported_successfully: 'Quiz imported successfully',

    // Hardcoded string fixes
    reconnected_as_host: 'Reconnected as host!',
    host_ended_session: 'Host ended the session',
    disconnected_count: '({0} disconnected)',
    please_wait_next_question: 'Please wait for the next question...',
    question_image_alt: 'Question Image',
    default_player_name: 'Player',
    question_regenerated: 'Question regenerated',
    failed_to_regenerate: 'Failed to regenerate: {0}',
    formatting_not_detected: 'Expected formatting not detected in this question',
    ollama_config_saved: 'Ollama: {0}',
    ollama_disabled: 'Ollama disabled',
    ollama_failed_save: 'Failed to save Ollama config',
    loading_models: 'Loading models...',
    no_models_found: 'No models found',
    excel_lib_not_available: 'Excel processing library not available',
    no_valid_excel_data: 'No valid data found in Excel file',
    failed_process_excel: 'Failed to process Excel file: {0}',
    answer_label: 'Answer:',
    tolerance_label: 'Tolerance:',
    downloaded_file: 'Downloaded: {0}',
    image_failed_to_load: 'Image failed to load',
    check_image_exists: 'Check if the image file exists',
    action_failed: 'Action failed',
    root_folder: '/ (Root)',
    enter_api_key_placeholder: 'Enter your API key',

    // Landing page (editorial redesign)
    landing_hero_eyebrow: 'A quiz platform for classrooms',
    landing_hero_title_lead: 'Quizzes for your class,',
    landing_hero_title_tail: 'in a',
    landing_hero_lede: 'turns any class into a multiplayer game. AI-generated questions, LaTeX equations, live results — running locally on your network, no external services.',
    landing_cta_create_quiz: 'Create a Quiz',
    landing_cta_quick_start: 'Quick start',
    landing_cta_note: 'No account · Free · Open source',
    landing_stat_languages: 'Languages',
    landing_stat_qtypes: 'Question types',
    landing_stat_players: 'Players',
    landing_stat_local: 'Local network',
    landing_joiners_suffix: 'students connected',
    landing_pin_title_em: 'Join',
    landing_pin_title_tail: 'the game',
    landing_pin_live: 'Live',
    landing_pin_name_placeholder: 'Your name in class',
    landing_pin_join: 'Enter the game',
    landing_pin_or: 'or scan',
    landing_pin_qr_title: 'Classroom QR code',
    landing_pin_qr_desc: 'Point your camera and join without typing anything.',
    landing_pin_qr_none_title: 'No active games',
    landing_pin_qr_none_desc: 'Start a game above to see its QR here.',
    landing_pin_qr_live_desc: 'Scan to join PIN #{0}.',
    landing_ticker_players: '{0} players',
    landing_howto_kicker: 'How it works',
    landing_howto_title_lead: 'Four steps from',
    landing_howto_title_em: 'silence',
    landing_howto_title_tail: 'to a connected classroom.',
    landing_howto_aside: 'Designed so a teacher can open their laptop, project the PIN, and get the whole class playing in under sixty seconds.',
    landing_step1_title: 'Create or generate your quiz',
    landing_step1_body: 'Write your questions, upload a CSV, or let Claude or a local model generate them from your lesson material.',
    landing_step2_title: 'Project the PIN',
    landing_step2_body: 'A 6-digit code and a QR. Students join from any device on the network — no apps, no accounts.',
    landing_step3_title: 'Play live',
    landing_step3_body: 'Synchronized timers, speed-based scoring, and a leaderboard projected for the whole class.',
    landing_step4_title: 'Analyze and export',
    landing_step4_body: 'PDF, Excel, per-question charts. Compare sessions of the same quiz across classes.',
    landing_qpreview_question: 'Question',
    landing_qpreview_qtext: 'What is the real solution of the equation',
    landing_qpreview_no_solution: 'No real solution',
    landing_qpreview_responses: '24 responses received',
    landing_qpreview_live_distribution: 'Live distribution',
    landing_features_kicker: 'Features',
    landing_features_title_lead: 'Built for the real',
    landing_features_title_em: 'classroom',
    landing_features_aside: 'Native support for math, code, and multimedia. Runs on your local network — no internet, no corporate filtering, no student accounts.',
    landing_feature1_title: 'Native LaTeX',
    landing_feature1_body: 'Equations, integrals, matrices, and mathematical symbols rendered with MathJax, directly in questions and options.',
    landing_feature2_title: 'AI generation',
    landing_feature2_body: 'Claude, local Ollama, HuggingFace. Paste a PDF or a topic and get a full quiz in seconds, with LaTeX and code auto-detected.',
    landing_feature3_title: 'Actually real-time',
    landing_feature3_body: 'Low-latency Socket.IO. Answers revealed automatically when the timer expires — no spoilers before.',
    landing_feature4_title: 'Local network, no internet',
    landing_feature4_body: 'Everything runs on your laptop. Students join via school WiFi. No external data, no accounts, no privacy risks.',
    landing_feature5_title: 'Code and syntax',
    landing_feature5_body: 'Highlights Python, JavaScript, C++, SQL snippets. Ideal for programming or CS courses.',
    landing_feature6_title: 'Exportable analytics',
    landing_feature6_body: 'Formatted PDF, Excel with 4 sheets (summary, questions, players, mistakes). Compare sessions of the same quiz across classes.',
    landing_qtypes_kicker: 'Question types',
    landing_qtypes_title: 'Five formats, one experience.',
    landing_qtypes_aside: 'From classic trivia to numeric entries with tolerance for physics and chemistry labs.',
    landing_qtype1_title: 'Multiple choice',
    landing_qtype1_body: 'Classic A / B / C / D with speed bonuses.',
    landing_qtype2_title: 'Multiple correct',
    landing_qtype2_body: 'Check all that apply — with partial credit.',
    landing_qtype3_title: 'True / False',
    landing_qtype3_body: 'Binary answer with instant feedback.',
    landing_qtype4_title: 'Numeric entry',
    landing_qtype4_body: 'Answers with configurable tolerance — ideal for physics and calculus.',
    landing_qtype4_tolerance: 'tolerance',
    landing_qtype5_title: 'Ordering',
    landing_qtype5_body: 'Drag and drop to reorder sequences or steps.',
    landing_demo_kicker: 'Try it',
    landing_demo_title_em: 'Play',
    landing_demo_title: 'a real question, right here.',
    landing_demo_aside: 'No screenshots: MathJax typesets the equation and highlight.js colors the code, using the same engine that runs in class.',
    landing_demo_code_q: 'What does this program print?',
    landing_demo_tf_q: 'Every continuous function on a closed interval [a, b] attains a maximum value on that interval.',
    landing_demo_num_q: 'An object starts from rest and accelerates at 9.8 m/s². What speed does it reach after 3.0 s?',
    landing_demo_correct: 'Correct!',
    landing_demo_wrong: 'Not quite — the correct answer is marked.',
    landing_demo_next: 'Next question',
    landing_ai_eyebrow: 'AI question generator',
    landing_ai_title_1: 'One',
    landing_ai_title_em: 'topic',
    landing_ai_title_2: 'ten questions,',
    landing_ai_title_3: 'zero work.',
    landing_ai_body: 'Write a concept, paste your lesson material, or upload a PDF. The generator auto-detects LaTeX and code and returns questions ready to play.',
    landing_ai_prompt_label: 'Prompt',
    landing_ai_prompt_text: 'Generate 1 multiple-choice question about the fundamental theorem of calculus, university level.',
    landing_ai_output_label: 'Generated output',
    landing_ai_output_text: 'If F is a continuous antiderivative of f on [a,b], then',
    landing_ai_output_equals: 'equals:',
    landing_cta_block_title_lead: 'Your next class starts with',
    landing_cta_block_title_em: 'a PIN',
    landing_cta_block_body: 'Free, open source, and runs on your laptop. No accounts, no limits.',
    landing_cta_block_primary: 'Create your first quiz',
    landing_cta_block_github: 'View on GitHub',
    landing_footer_about_title: 'About',
    landing_footer_about_body: 'Open-source interactive quiz platform, designed for classrooms, with native math and AI generation.',
    landing_footer_product_title: 'Product',
    landing_footer_host: 'Create a Quiz',
    landing_footer_join: 'Join a game',
    landing_footer_ai: 'AI generator',
    landing_footer_qtypes: 'Question types',
    landing_footer_resources_title: 'Resources',
    landing_footer_docs: 'Documentation',
    landing_footer_issues: 'Issues',
    landing_footer_features: 'Features',
    landing_footer_howto: 'How it works',
    landing_footer_connect_title: 'Connect',
    landing_footer_license: 'MIT License',

    // Editor 2a Stage B: question sidebar + preview column
    qs_no_text: 'No text',
    minutes_short: 'MIN',
    try_full_quiz: 'Try the full quiz',

    // Editor 2a Stage C: question form restyle
    type_label: 'Type',
    difficulty_label: 'Difficulty',
    points_label: 'Points',
    concepts_max: '· max 5',
    correct_pill: '✓ Correct',
    mark_correct_pill: 'Mark',
    poll_mode_label: 'Poll — no correct answer',
    poll_mode_hint: 'Students answer True or False; nobody is marked right or wrong and no points are awarded.',
    poll_answer_recorded: 'Answer recorded — thanks for your response!',
    poll_no_correct_answer: 'Poll — no correct answer',
    image_label: 'Image',
    image_drop_hint: 'drag & drop or click',
    explanation_hint: 'shown after answering',
    duplicate_question: 'Duplicate',
    delete_question: 'Delete',

    // Editor 2a Stage D — validation surfaces + autosave stamp
    val_no_text: 'no text',
    val_no_correct: 'no correct answer',
    val_correct_empty: 'correct option is empty',
    val_too_few_options: 'needs 2 options',
    val_no_numeric: 'no numeric answer',
    val_strip_one: '1 question needs attention before creating the room',
    val_strip_many: '{0} questions need attention before creating the room',
    val_go_first: 'Go to first',
    val_dismiss: 'Dismiss',
    quiz_status_title: 'Quiz status',
    val_ready_one: '1 question ready',
    val_ready_many: '{0} questions ready',
    val_pill_one: '1 warning · view',
    val_pill_many: '{0} warnings · view',
    val_option_empty_helper: 'Write the option or remove it before creating the room',
    editor_saving: 'Saving…',
    editor_saved: 'Saved',

    // Game settings, team discussion and consensus (were missing: the markup
    // fallbacks meant every locale rendered these in English)
    game_settings: 'Game Settings',
    same_time_for_all: 'Same Time for All',
    rematch: 'Rematch',
    team_discussion: 'Team Discussion',
    discussion_activity: 'Discussion Activity',
    current_proposals: 'Current Proposals',
    allow_free_chat: 'Allow free-form chat (for trusted groups)',
    consensus_of: 'of',
    threshold: 'threshold',
    unanimous: '100% (Unanimous)',
    ollama_server_url: 'Ollama Server:'
};
