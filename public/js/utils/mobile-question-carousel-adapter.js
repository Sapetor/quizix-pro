/**
 * Mobile Question Carousel Adapter - Uses UnifiedCarousel for navigation
 * Replaces mobile-question-carousel.js with simplified implementation
 * Handles quiz editor question navigation on mobile
 *
 * NOTE: This carousel has unique behavior (DOM cloning, form sync) that cannot
 * be fully unified. We keep custom logic here while using UnifiedCarousel for
 * basic navigation when possible.
 */
...
class MobileQuestionCarousel {
    ...
    /**
     * Update carousel with current questions
     */
    updateCarousel() {
        if (!this.isCarouselActive || !this.questionsWrapper) return;

        // Get all questions from the main container
    ...
    /**
     * Show current question (custom implementation for form cloning)
     */
    showCurrentQuestion() {
